...
def bear_room(): # defines bear_room function which takes no parameter
    ...
    while True: # begins while-loop which runs whilst assigned Boolean state is True
        ...
        if choice == "take honey": # if string assigned to choice is exactly equal to "take honey" then run code block below
            dead("The bear looks at you then slaps your face off.") # calls dead function with string parameter (see dead function defined below)
        elif choice == "taunt bear" and not bear_moved: # else (skip above code block) and if string assigned to choice is exactly equal to "taunt bear" AND Boolean state is not False (currently state assigned to bear_moved is False) then run following code block
            print("The bear has moved from the door.") # prints string
            print("You can go through it now.") # prints string
            bear_moved = True # change Boolean state to True and assign to variable name bear_moved so runs loop again resulting in another prompt to the user for input
            ...
            dead("The bear gets pissed off and chews your legs off.") # calls dead function with string as parameter (see dead function defined below)
        elif choice == "open door" and bear_moved: # else (skips all three code blocks above) and if string assigned to choice is exactly equal to "open door" (second iteration of while-loop) AND Boolean state is True (bear has moved) then run following code block
            gold_room() # calls gold_room function with no parameter (defined above)
        else: # non of above requirements met then all previous code blocks skipped and following code block runs
            print("I got no idea what that means.") # prints string which implies that user has inputted a string that is not recognised. bear_moved will remain False? so while-loop does not run again? Ideally we would prompt for input again until user lands on something that is recognised
